...
const DraggableTabs = (props) => {
  const { children, panes, setNewPanes } = props;
  ...
  // panes.current
  const newPanes = panes.slice().sort((a, b) => {
  ...
  setNewPanes(newPanes);
  return (
    <DndProvider backend={HTML5Backend}>
      <Tabs renderTabBar={renderTabBar} {...props}>
        {newPanes.map((pane) => tabs[pane.key])}
      </Tabs>
    </DndProvider>
  );
};
...
function MainPage(props) {
  const getAllTables = useStoreActions(
    (actions) => actions.database.getAllTables
  );
  const deleteTable = useStoreActions(
    (actions) => actions.database.deleteTable
  );

  // const errorMessageDatabase = useStoreState(
  //   (state) => state.database.errorMessage
  // );
  // const setErrorMessageDatabase = useStoreActions(
  ...
  const removeTablePoint = useStoreActions(
    (actions) => actions.database.removeTablePoint
  );
  const removePointHistory = useStoreActions(
    (actions) => actions.database.removePointHistory
  );
  const pointHisList = useStoreState((state) => state.database.pointHisList);

  const addPointHistory = useStoreActions(
    (actions) => actions.database.addPointHistory
  );

  const pointTrendList = useStoreState(
    (state) => state.database.pointTrendList
  );
  // const pointTrendDataList = useStoreState(
  //   (state) => state.database.pointTrendDataList
  // );
  const removePointTrend = useStoreActions(
    (actions) => actions.database.removePointTrend
  );
  const addPointTrend = useStoreActions(
    (actions) => actions.database.addPointTrend
  );
  ...
  const setErrorMessage = useStoreActions(
    (actions) => actions.message.setErrorMessage
  );

  const successMessage = useStoreState((state) => state.message.successMessage);
  const setSuccessMessage = useStoreActions(
    (actions) => actions.message.setSuccessMessage
  );

  const setCurTable = useStoreActions(
    (actions) => actions.database.setCurTable
  );
  ...
  const dbClick = (e, info) => {
    let find = false;
    if (info.title.indexOf('表(') >= 0) return;
    panes.current.forEach((element) => {
      if (element.key === 'TestPage' && info.key === 'TestPage') {
        ...
        element.key === testMapFuncPage &&
        info.key == testMapFuncPage
      ) {
        ...
        element.key === 'DataTypeEdit' &&
        info.key === 'DataTypeEdit'
      ) {
        treeState.activeKey = 'DataTypeEdit';
        setTreeState({ ...treeState });
        find = true;
      } else if (
        element.key === alarmAreaEditPage &&
        info.key == alarmAreaEditPage
      ) {
        treeState.activeKey = alarmAreaEditPage;
        setTreeState({ ...treeState });
        find = true;
      } else if (
        element.key === alarmGroupEditPage &&
        info.key == alarmGroupEditPage
      ) {
        treeState.activeKey = alarmGroupEditPage;
        setTreeState({ ...treeState });
        find = true;
      } else if (
        element.key === alarmSystemEditPage &&
        info.key == alarmSystemEditPage
      ) {
    ...
    if (info.key === 'TestPage') {
      ...
      setTreeState({ ...treeState });
      return;
    } else if (info.key === alarmAreaEditPage) {
      //报警区域编辑
      panes.current.push({
        title: '报警区域编辑',
        key: alarmAreaEditPage,
        closable: true,
      ...
      setTreeState({ ...treeState });
      return;
    } else if (info.key === alarmGroupEditPage) {
      //报警组编辑
      panes.current.push({
        title: '报警组编辑',
        key: alarmGroupEditPage,
        closable: true,
      ...
      setTreeState({ ...treeState });
      return;
    } else if (info.key === alarmSystemEditPage) {
      //报警组编辑
      panes.current.push({
        title: '报警系统编辑',
        key: alarmSystemEditPage,
        closable: true,
      ...
      setTreeState({ ...treeState });
      return;
    } else if (info.key === alarmLevelEditPage) {
      //报警级别编辑
      panes.current.push({
        title: '报警级别编辑',
        key: alarmLevelEditPage,
        closable: true,
  ...
  const openTrend = useCallback(
    (record, pointTrendList) => {
      ...
      panes.current.push({
        ...
        key: 'trend-' + record.FullName,
        closable: true,
        name: 'trend-' + record.FullName,
        type: 'trend',
  ...
  const onWindowResize = useCallback(() => {
    treeState.treeHeight =
      document.body.clientHeight - GlobalSetting.HeaderHeight - 50;
    setTreeState({ ...treeState });
  }, [treeState]);
  //重新加载
  const freshIpPort = () => {
  ...
  const remove = (targetKey) => {
    ...
    if (removeName.indexOf('trend-') >= 0) removePointTrend(removeName);
    else removePointHistory(removeName);
  ...
  const deleteTableClick = () => {
    ...
    confirm({
      ...
      onOk() {
        ...
        panes.current = panes.current.filter(
          (pane) => pane.title !== menuState.menuKey
        );
        setTreeState({ ...treeState });
      },
      onCancel() {},
  ...
  const closeAllTabClick = () => {
    ...
    while (panes.current.length > 0) panes.current.pop();
    handleTabMenuClose();
  };
  //关闭其它的tab页
  const closeOtherTabClick = (e) => {
    ...
    panes.current = panes.current.filter(
      (pane) => pane.key === tabMenuState.tab.key
    );
          ...
          <Col span={6}>
            ...
            >
              ...
              >
                <Tooltip title='退出登录'>
                  {' '}
                  <LogoutOutlined rotate={-90} style={{ fontSize: '20px' }} />
                </Tooltip>
              </div>
            </div>
          </Col>
      ...
      <Layout style={{ backgroundColor: 'white' }}>
        <Sider style={{ backgroundColor: 'white' }}>
          ...
          >
            <DirectoryTree
              treeData={
                filterState.filterTableList
                  ? filterState.filterTableList
                  : [{ title: '表', key: '-1', isLeaf: false }]
              }
              height={treeState.treeHeight}
            ...
            <Menu
              ...
              anchorPosition={
                tabMenuState.menuMouseY !== null &&
                tabMenuState.menuMouseX !== null
                  ? {
                      top: tabMenuState.menuMouseY,
                      left: tabMenuState.menuMouseX,
                    }
                  : undefined
              }
            >
            ...
            >
              <MenuItem
                ...
                disabled={menuState.addTableMenuDisable}
              >
                <Space>
                  <PlusOutlined />
                  <span>添加表</span>
              ...
              <MenuItem
                ...
                disabled={menuState.deleteTableMenuDisable}
              >
                <Space>
                  <DeleteOutlined />
                  <span>删除表</span>
              ...
              <MenuItem
                onClick={freshTableClick}
                disabled={menuState.freshTableMenuDisable}
              >
                <Space>
                  <ReloadOutlined />
                  <span>刷新表</span>
            ...
            <AppendTableModal
              visible={treeState.appendTableModalVisable}
              setVisible={setAppendTableModalVisible}
            ></AppendTableModal>
          </div>
        ...
        <Content style={{ backgroundColor: 'white', margin: '5px' }}>
          <DraggableTabs
            ...
            onEdit={onEdit}
            panes={panes.current}
            setNewPanes={(val) => (panes.current = val)}
          >
