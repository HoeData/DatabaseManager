...
function PointInfoModal(props) {
    ...
    const insertPoint = useStoreActions((actions) => actions.database.insertPoint);
    ...
    const CheckLimit=()=>{
        if(state.Type==1)//uint8 对应 byte  
        {
            return CheckUpLow(0,255);
        }
        else  if(state.Type==2)//int8 对应 sbyte  
        {
            return CheckUpLow(-128,127);
        }  
        else  if(state.Type==4)//uint16 对应 uint16  
        {
            return CheckUpLow(0,65535);
        } 
        else  if(state.Type==5)//int16 对应 int16   
        {
            return CheckUpLow(-32768,32767);
        }
        else  if(state.Type==6)//uint32 对应 uint32  
        {
            return CheckUpLow(0,4294967295);
        }
        else  if(state.Type==7)//int32 对应 int32  
        {
            return CheckUpLow(-2147483648,2147483647);
        }
        else  if(state.Type==8)//int64 对应 int64 
        {
            return CheckUpLow(-2147483648,2147483647);
        } 
        else  if(state.Type==8)//int64 对应 int64 
        ...
        else  if(state.Type==9)//real16 对应 single 
        {
            return CheckUpLow(-3.402823e38,+3.402823e38);
        }  
        else  if(state.Type==10)//real32 对应 float 
        {
            return CheckUpLow(-3.402823e38,+3.402823e38);
        } 
        else  if(state.Type==11)//real64 对应 double 
        {
    ...
    return <Modal onOk={onModalOkClick} mask={true} maskClosable={false}
        okText='确定'
        cancelText='取消'
        onCancel={onModalCancelClick} visible={visible} title={state.title}>
        <span style={{ margin: spanMargin, width: '100px' }}>测点:</span>
        <Input style={{ width: inputWidth }} value={state.TagName} onChange={tagChange}></Input>
        <div style={{ margin: divMargin, width: '100px' }}></div>
        <span style={{ margin: spanMargin, width: '100px' }}>类型:</span>
        <Select style={{ width: inputWidth }} value={state.Type} onChange={changeType}>
            {getDataTypes()}
        </Select>
        <div style={{ margin: divMargin }}></div>
        <span style={{ margin: spanMargin, width: '100px' }} >单位:</span>
        <Input style={{ width: inputWidth }} value={state.TagUnit} onChange={unitChange}></Input>
        <div style={{ margin: divMargin }}></div>
        {state.Type!=0?<span style={{ margin: spanMargin, width: '100px' }}>上限:</span>:null}
        {state.Type!=0? <InputNumber  disabled={state.Type==0} style={{ width: inputWidth }} value={state.UpperLimit} 
        onChange={upLimitChange}></InputNumber>:null}
        {state.Type!=0? <div style={{ margin: divMargin }}></div> :null}
        {state.Type!=0? <span style={{ margin: spanMargin, width: '100px' }}>下限:</span>:null}
        {state.Type!=0? <InputNumber  disabled={state.Type==0} style={{ width: inputWidth }} value={state.LowerLimit} 
        onChange={lowLimitChange} ></InputNumber>:null}
        {state.Type!=0? <div style={{ margin: divMargin }}></div>:null}
        <span style={{ margin: spanMargin, width: '100px' }}>描述:</span>
        <Input style={{ width: inputWidth, height: '50px' }} multiple value={state.TagDesc} onChange={descChange}></Input>
        <div style={{ margin: divMargin }}></div>
        <span style={{ margin: spanMargin, width: '100px' }}>备注:</span>
        <Input style={{ width: inputWidth, height: '50px' }} multiple value={state.Remark} onChange={remarkChange}></Input>
        {state.alertVisible ? (
